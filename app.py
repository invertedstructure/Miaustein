...
def projection_pattern_hints(
    ...
    state_walk: list[str] = []
    for row in trace:
        for key in ("before_state_sig8", "after_state_sig8"):
            value = row.get(key)
            if value is not None:
                state_walk.append(str(value))

    states_visited: list[str] = []
    for state_sig in state_walk:
        if state_sig not in states_visited:
    ...
    return {
        ...
        "repeated_states": "yes" if len(state_walk) != len(set(state_walk)) else "no",
