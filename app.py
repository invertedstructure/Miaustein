...
def _without_self_hash_fields(obj: Any) -> Any:
    if isinstance(obj, dict):
        return {
            ...
            if k not in {"sig8", "receipt_sig8", "external_check_sig8", "candidate_surface_sig8"}
        }
...
# -----------------------------------------------------------------------------
# Capability Layer 2: bounded candidate search surface.
#
# This is a read-only post-halt surface. It enumerates candidate continuations
# from a fixed vocabulary, rejects invalid candidates, and tests a tiny local
# continuation without mutating state, regime, receipt, or the move registry.
# Candidate appearance is not candidate authority.
# -----------------------------------------------------------------------------
LAYER2_SURFACE_ID_V0 = "bounded_candidate_search_surface.v0"
LAYER2_DECLARED_DIRECTION_V0 = "post_stop_needs_new_move"
LAYER2_SEARCH_RADIUS_V0 = 1
LAYER2_MUTATION_VOCABULARY_V0 = [
    "draft_move_skeleton_readout",
    "halt_dictionary_readout",
    "taxonomy_readout",
]
LAYER2_FORBIDDEN_MUTATION_KINDS_V0 = {
    "autonomous_registry_append",
    "architecture_redesign",
    "dynamic_regime_update",
    "learned_move_admission",
}
LAYER2_CANDIDATE_TEMPLATES_V0: list[dict[str, Any]] = [
    {
        "candidate_id": "candidate.draft_smallest_missing_move.v0",
        "candidate_move_id": "draft_smallest_missing_move.v0",
        "priority": 0,
        "direction": LAYER2_DECLARED_DIRECTION_V0,
        "search_radius": 1,
        "mutation_kind": "draft_move_skeleton_readout",
        "declared_trigger_halt": "STOP_NEEDS_NEW_MOVE",
        "requires_final_status": "stop_needs_new_move_halt_vocabulary_bound",
        "readout_only": True,
        "smallest_honest_reading": "draft a smallest missing move skeleton for human-authored patch review only",
        "must_not_impersonate": [
            "registered_move",
            "admitted_move",
            "registry_patch",
    ...
    {
        "candidate_id": "candidate.add_halt_dictionary_readout.v0",
        "candidate_move_id": "add_halt_dictionary_readout.v0",
        "priority": 1,
        "direction": LAYER2_DECLARED_DIRECTION_V0,
        "search_radius": 1,
        "mutation_kind": "halt_dictionary_readout",
        "declared_trigger_halt": "STOP_TAXONOMY_GAP",
        "requires_final_status": "stop_needs_new_move_halt_vocabulary_bound",
        "readout_only": True,
        "smallest_honest_reading": "readout-only halt dictionary note; rejected here because the observed halt is missing-move pressure",
        "must_not_impersonate": ["halt_admission", "taxonomy_upgrade", "registered_move"],
    },
    {
        "candidate_id": "candidate.taxonomy_upgrade.v0",
        "candidate_move_id": "taxonomy_upgrade.v0",
        "priority": 2,
        "direction": LAYER2_DECLARED_DIRECTION_V0,
        "search_radius": 1,
        "mutation_kind": "taxonomy_readout",
        "declared_trigger_halt": "STOP_TAXONOMY_GAP",
        "requires_final_status": "stop_needs_new_move_halt_vocabulary_bound",
        "readout_only": True,
        "smallest_honest_reading": "taxonomy readout candidate; rejected here because STOP_NEEDS_NEW_MOVE is not taxonomy-gap pressure",
        "must_not_impersonate": ["truth_creation", "move_admission", "registry_patch"],
    },
    {
        "candidate_id": "candidate.autonomous_registry_append.v0",
        "candidate_move_id": "autonomous_registry_append.v0",
        "priority": 3,
        "direction": LAYER2_DECLARED_DIRECTION_V0,
        "search_radius": 1,
        "mutation_kind": "autonomous_registry_append",
        "declared_trigger_halt": "STOP_NEEDS_NEW_MOVE",
        "requires_final_status": "stop_needs_new_move_halt_vocabulary_bound",
        "readout_only": False,
        "smallest_honest_reading": "intentionally invalid candidate used to verify rejection of autonomous registry evolution",
        "must_not_impersonate": ["lawful_candidate", "admitted_move", "human_patch"],
    },
    {
        "candidate_id": "candidate.architecture_redesign.v0",
        "candidate_move_id": "architecture_redesign.v0",
        "priority": 4,
        "direction": "architecture_widening",
        "search_radius": 99,
        "mutation_kind": "architecture_redesign",
        "declared_trigger_halt": "STOP_NEEDS_NEW_MOVE",
        "requires_final_status": "stop_needs_new_move_halt_vocabulary_bound",
        "readout_only": False,
        "smallest_honest_reading": "intentionally invalid candidate used to verify rejection of widening",
        "must_not_impersonate": ["bounded_search", "local_candidate", "registered_move"],
    },
]


def _layer2_base_sigs(
    final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    return {
        "state_sig8": sig8(final_state) if isinstance(final_state, dict) else None,
        "regime_sig8": sig8(regime) if isinstance(regime, dict) else None,
        "receipt_sig8": receipt.get("receipt_sig8") if isinstance(receipt, dict) else None,
    }


def _validate_layer2_candidate_template_v0(
    template: dict[str, Any], *, final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> list[str]:
    errors: list[str] = []
    halt_code = receipt.get("halt_code") if isinstance(receipt, dict) else None
    move_registry = regime.get("move_registry") if isinstance(regime, dict) else None
    halt_codes = regime.get("halt_codes") if isinstance(regime, dict) else None

    if halt_code != "STOP_NEEDS_NEW_MOVE":
        errors.append("trigger_halt_not_STOP_NEEDS_NEW_MOVE")
    if template.get("declared_trigger_halt") != halt_code:
        errors.append(
            "candidate_trigger_halt_mismatch="
            + str(template.get("declared_trigger_halt"))
            + "!="
            + str(halt_code)
        )
    if template.get("direction") != LAYER2_DECLARED_DIRECTION_V0:
        errors.append("direction_out_of_scope=" + str(template.get("direction")))
    if template.get("search_radius") != LAYER2_SEARCH_RADIUS_V0:
        errors.append("search_radius_out_of_bounds=" + str(template.get("search_radius")))
    if template.get("mutation_kind") not in LAYER2_MUTATION_VOCABULARY_V0:
        errors.append("mutation_kind_not_in_fixed_vocabulary=" + str(template.get("mutation_kind")))
    if template.get("mutation_kind") in LAYER2_FORBIDDEN_MUTATION_KINDS_V0:
        errors.append("forbidden_mutation_kind=" + str(template.get("mutation_kind")))
    if template.get("readout_only") is not True:
        errors.append("candidate_not_readout_only")

    candidate_move_id = template.get("candidate_move_id")
    if not isinstance(candidate_move_id, str) or not candidate_move_id:
        errors.append("candidate_move_id_missing")
    else:
        if isinstance(move_registry, list) and candidate_move_id in move_registry:
            errors.append("candidate_already_registered=" + candidate_move_id)
        if isinstance(halt_codes, list) and candidate_move_id in halt_codes:
            errors.append("candidate_impersonates_halt_code=" + candidate_move_id)

    required_status = template.get("requires_final_status")
    if isinstance(required_status, str):
        if not isinstance(final_state, dict) or final_state.get("status") != required_status:
            errors.append("required_final_status_missing=" + required_status)

    guards = template.get("must_not_impersonate")
    if not isinstance(guards, list) or not all(isinstance(x, str) and x for x in guards):
        errors.append("must_not_impersonate_missing")
    else:
        required = {"registered_move", "admitted_move", "registry_patch"}
        if template.get("candidate_id") == "candidate.draft_smallest_missing_move.v0" and not required.issubset(set(guards)):
            errors.append("candidate_authority_guards_missing")

    return errors


def _layer2_local_continuation_test_v0(
    template: dict[str, Any], *, final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    before_sigs = _layer2_base_sigs(final_state, regime, receipt)
    candidate_move_id = template.get("candidate_move_id")
    move_skeleton = {
        "candidate_move_id": candidate_move_id,
        "candidate_authority": "none_readout_only",
        "applies_when": [
            "receipt.halt_code == STOP_NEEDS_NEW_MOVE",
            "final_state.status == stop_needs_new_move_halt_vocabulary_bound",
        ],
        "action": "emit human-reviewable move skeleton only",
        "emits": ["candidate_readout"],
        "may_halt": ["STOP_NEEDS_NEW_MOVE", "NO_APPLICABLE_MOVE", "INVALID_STATE"],
        "must_not_mutate": ["state", "regime", "receipt", "move_registry"],
    }
    after_sigs = _layer2_base_sigs(final_state, regime, receipt)
    unchanged = before_sigs == after_sigs
    return {
        "test_id": "layer2.radius1.local_continuation_test.v0",
        "tested_steps": 1,
        "radius_bound": LAYER2_SEARCH_RADIUS_V0,
        "verdict": "PASS" if unchanged else "FAIL",
        "state_sig8_before": before_sigs["state_sig8"],
        "state_sig8_after": after_sigs["state_sig8"],
        "regime_sig8_before": before_sigs["regime_sig8"],
        "regime_sig8_after": after_sigs["regime_sig8"],
        "receipt_sig8_before": before_sigs["receipt_sig8"],
        "receipt_sig8_after": after_sigs["receipt_sig8"],
        "move_skeleton": move_skeleton,
    }


def _layer2_candidate_record_v0(
    template: dict[str, Any], *, final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    rejection_reasons = _validate_layer2_candidate_template_v0(
        template, final_state=final_state, regime=regime, receipt=receipt
    )
    valid = not rejection_reasons
    local_test = (
        _layer2_local_continuation_test_v0(template, final_state=final_state, regime=regime, receipt=receipt)
        if valid
        else {
            "test_id": "layer2.radius1.local_continuation_test.v0",
            "tested_steps": 0,
            "radius_bound": LAYER2_SEARCH_RADIUS_V0,
            "verdict": "NOT_RUN_REJECTED_CANDIDATE",
        }
    )
    return {
        ...
        "search_radius": template.get("search_radius"),
        "mutation_kind": template.get("mutation_kind"),
        "smallest_honest_reading": template.get("smallest_honest_reading"),
        "must_not_impersonate": template.get("must_not_impersonate"),
        "valid": valid,
        "rejection_reasons": rejection_reasons,
        "local_continuation_test": local_test,
        "admission_status": "NOT_ADMITTED_CANDIDATE_ONLY",
    }


def build_bounded_candidate_search_surface_v0(
    *, final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    before_sigs = _layer2_base_sigs(final_state, regime, receipt)
    candidates = [
        _layer2_candidate_record_v0(template, final_state=final_state, regime=regime, receipt=receipt)
        for template in LAYER2_CANDIDATE_TEMPLATES_V0
    ]
    valid_candidates = [c for c in candidates if c.get("valid") is True]
    selected = sorted(valid_candidates, key=lambda c: (c.get("priority", 999), str(c.get("candidate_id"))))[:1]
    after_sigs = _layer2_base_sigs(final_state, regime, receipt)
    active = isinstance(receipt, dict) and receipt.get("halt_code") == "STOP_NEEDS_NEW_MOVE"
    surface = {
        "surface_id": LAYER2_SURFACE_ID_V0,
        "role": "capability_layer_2_read_only_candidate_search",
        "verdict": "PASS" if active and selected and before_sigs == after_sigs else "INACTIVE_OR_FAIL",
        "trigger_halt": receipt.get("halt_code") if isinstance(receipt, dict) else None,
        "authority": "zero_candidate_authority",
        "candidate_appearance_rule": "candidate appearance is not candidate authority",
        "bounds": {
            "declared_continuation_direction": LAYER2_DECLARED_DIRECTION_V0,
            "search_radius": LAYER2_SEARCH_RADIUS_V0,
            "fixed_mutation_vocabulary": LAYER2_MUTATION_VOCABULARY_V0,
        },
        "forbidden": [
            "registry mutation",
            "move admission",
        ...
        "input_sigs": before_sigs,
        "output_sigs": after_sigs,
        "candidates": candidates,
        "selected_candidate_id": selected[0].get("candidate_id") if selected else None,
        "registry_status": "UNCHANGED_NOT_MUTATED",
        "state_status": "UNCHANGED_NOT_MUTATED",
        "receipt_status": "UNCHANGED_NOT_MUTATED",
    }
    surface["candidate_surface_sig8"] = sig8(surface)
    return surface


def verify_bounded_candidate_search_surface_v0(
    *, surface: dict[str, Any], final_state: dict[str, Any] | None, regime: dict[str, Any], receipt: dict[str, Any]
) -> dict[str, Any]:
    check_id = "verify_bounded_candidate_search_surface.v0"
    errors: list[str] = []
    if not isinstance(surface, dict):
        errors.append("surface_not_object")
        surface = {}
    if surface.get("surface_id") != LAYER2_SURFACE_ID_V0:
        errors.append("surface_id_mismatch")
    if surface.get("authority") != "zero_candidate_authority":
        errors.append("candidate_authority_not_zero")
    if surface.get("trigger_halt") != "STOP_NEEDS_NEW_MOVE":
        errors.append("trigger_halt_not_stop_needs_new_move")
    if surface.get("registry_status") != "UNCHANGED_NOT_MUTATED":
        errors.append("registry_status_not_unchanged")
    if surface.get("state_status") != "UNCHANGED_NOT_MUTATED":
        errors.append("state_status_not_unchanged")
    if surface.get("receipt_status") != "UNCHANGED_NOT_MUTATED":
        errors.append("receipt_status_not_unchanged")

    expected_sigs = _layer2_base_sigs(final_state, regime, receipt)
    if surface.get("input_sigs") != expected_sigs:
        errors.append("input_sigs_mismatch")
    if surface.get("output_sigs") != expected_sigs:
        errors.append("output_sigs_mismatch")
    ...
    for index, candidate in enumerate(candidates):
        if not isinstance(candidate, dict):
            ...
            continue
        if candidate.get("admission_status") != "NOT_ADMITTED_CANDIDATE_ONLY":
            errors.append(f"candidate_admission_status_invalid={candidate.get('candidate_id')}")
        if candidate.get("valid") is True:
            valid_candidates.append(candidate)
            if candidate.get("direction") != LAYER2_DECLARED_DIRECTION_V0:
                errors.append(f"valid_candidate_direction_invalid={candidate.get('candidate_id')}")
            if candidate.get("search_radius") != LAYER2_SEARCH_RADIUS_V0:
                errors.append(f"valid_candidate_radius_invalid={candidate.get('candidate_id')}")
            if candidate.get("mutation_kind") not in LAYER2_MUTATION_VOCABULARY_V0:
                errors.append(f"valid_candidate_mutation_vocab_invalid={candidate.get('candidate_id')}")
            local_test = candidate.get("local_continuation_test")
            if not isinstance(local_test, dict) or local_test.get("verdict") != "PASS":
                errors.append(f"valid_candidate_local_test_not_pass={candidate.get('candidate_id')}")
            elif local_test.get("tested_steps") != LAYER2_SEARCH_RADIUS_V0:
                errors.append(f"valid_candidate_test_radius_invalid={candidate.get('candidate_id')}")
        else:
            if not candidate.get("rejection_reasons"):
                errors.append(f"invalid_candidate_missing_rejection_reason={candidate.get('candidate_id')}")

    expected_selected = None
    if valid_candidates:
        expected_selected = sorted(valid_candidates, key=lambda c: (c.get("priority", 999), str(c.get("candidate_id"))))[0].get("candidate_id")
    if surface.get("selected_candidate_id") != expected_selected:
        errors.append("selected_candidate_mismatch")

    forbidden_hits = _autonomous_continuation_field_hits(surface, "layer2_surface")
    if forbidden_hits:
        errors.append("forbidden_autonomous_field_present=" + ",".join(sorted(forbidden_hits)))

    surface_sig8 = surface.get("candidate_surface_sig8")
    if not isinstance(surface_sig8, str) or not surface_sig8:
        errors.append("candidate_surface_sig8_missing")
        recomputed = None
    else:
        recomputed = sig8(surface)
        if surface_sig8 != recomputed:
            errors.append(f"candidate_surface_sig8_mismatch: surface={surface_sig8!r}; recomputed={recomputed!r}")

    return _external_check_result({
        "check_id": check_id,
        "role": "layer_2_surface_verifier",
        "verdict": "PASS" if not errors else "FAIL",
        "errors": errors,
        "input_candidate_surface_sig8": surface.get("candidate_surface_sig8"),
        "recomputed_candidate_surface_sig8": recomputed,
        "checked_claims": [
            "candidate surface has zero authority",
            "trigger halt is STOP_NEEDS_NEW_MOVE",
            "state/regime/receipt signatures are unchanged",
            "valid candidates stay inside direction/radius/vocabulary bounds",
            "invalid candidates carry rejection reasons",
            "valid candidates pass radius-1 local continuation tests",
            "selected candidate is the lowest-priority valid candidate",
            "no autonomous continuation fields appear in the surface",
        ],
    })
...
def main() -> None:
    ...
    if st.button("Run lawful move machine"):
        ...
        layer2_candidate_surface = build_bounded_candidate_search_surface_v0(
            receipt=receipt,
            final_state=final_state,
            regime=regime_for_external,
        )
        layer2_surface_verifier = verify_bounded_candidate_search_surface_v0(
            surface=layer2_candidate_surface,
            receipt=receipt,
            final_state=final_state,
            regime=regime_for_external,
        )
        ...
        st.json(
            {
                ...
                "layer2_candidate_surface_verifier": layer2_surface_verifier,
            }
        )

        st.subheader("Capability Layer 2 — bounded candidate search surface")
        st.caption("Read-only candidate surface after STOP_NEEDS_NEW_MOVE; no registry mutation or admission.")
        st.json(layer2_candidate_surface)
