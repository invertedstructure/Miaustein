...
def main() -> None:
    ...
    if isinstance(regime, dict) and isinstance(state, dict) and not load_errors:
        ...
        if regime_errors:
            ...
            st.subheader("Candidate Search v0 — read-only continuation surface")
            candidate_readout = candidate_search_readout(state, regime)
            selected_candidate_id = candidate_readout.get("selected_candidate_id")
            st.caption("Capability Layer 2 foothold: enumerate lawful candidates without mutating state. Layer 4 foothold: expose typed stop/block reasons without admitting new moves.")
            st.json({
                ...
                "readout_sig8": candidate_readout.get("readout_sig8"),
            })
            with st.expander("All candidate records", expanded=False):
    ...
    if st.button("Run lawful move machine"):
        ...
        if isinstance(final_state, dict) and isinstance(regime, dict):
            st.markdown("**Candidate Search v0 on final state — read-only**")
            st.json(candidate_search_readout(final_state, regime))
