...
def post_halt_continuation_reason_validation_errors_v0(
    ...
    if set(APPROVED_DRAFTABLE_MOVE_SHAPES_V0) != expected_approved_shapes:
        errors.append("approved_shape_set_not_exactly_expected_three")
...
def _accepted_post_halt_continuation_reason_move_phase_closed_v0(
    ...
    return (
        ...
        and state.get("status") == NEXT_SELECTED_POST_HALT_REASON_OUTCOME_V0
    )
    ...
    *,
    ...
    if not isinstance(post_halt_reason_readout, dict):
        ...
        for field, expected in required_post_halt_values.items():
            if post_halt_reason_readout.get(field) != expected:
                blockers.append(f"post_halt_reason_{field}_mismatch")
    if isinstance(move_registry, list) and TERMINAL_REASON_SPECIFICITY_MOVE_ID_V0 in move_registry:
        blockers.append("terminal_specificity_move_already_in_move_registry")
    if TERMINAL_REASON_SPECIFICITY_MOVE_ID_V0 in MOVE_ORDER_V0:
    ...
    if TERMINAL_REASON_SPECIFICITY_MOVE_ID_V0 in APPROVED_DRAFTABLE_MOVE_SHAPES_V0:
