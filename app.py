...
def apply_validate_registry_exhaustion_witness(
    ...
    if post_applicable:
        new_state = append_history(
            state,
            {
                ...
                "checkpoint_code": None,
                "halt_code": "INVALID_STATE",
                "reason": "post_state_still_has_applicable_move="
                + ",".join(row.get("move_id", "<unknown>") for row in post_applicable),
            },
        )
        new_state["status"] = "halted_invalid"
        return new_state, "INVALID_STATE", None
    ...
    if dry_halt_code != "STOP_NEEDS_NEW_MOVE":
        new_state = append_history(
            state,
            {
                "move_id": "validate_registry_exhaustion_witness.v0",
                ...
                "checkpoint_code": None,
                "halt_code": "INVALID_STATE",
                "reason": "post_state_terminal_halt_mismatch=" + dry_halt_code,
            },
        )
        new_state["status"] = "halted_invalid"
        return new_state, "INVALID_STATE", None

    forbidden_completion_words = ("proof complete", "proof closure", "engine complete", "success", "done")
    if any(word in dry_reason.lower() for word in forbidden_completion_words):
        new_state = append_history(
            state,
            {
                "move_id": "validate_registry_exhaustion_witness.v0",
                ...
                "checkpoint_code": None,
                "halt_code": "INVALID_STATE",
                "reason": "exhaustion_reason_impersonates_completion",
            },
        )
        new_state["status"] = "halted_invalid"
        return new_state, "INVALID_STATE", None
...
def classify_no_applicable_move(state: dict[str, Any], regime: dict[str, Any]) -> tuple[str, str]:
    if history_has_move(state, "validate_registry_exhaustion_witness.v0"):
        return (
            "STOP_NEEDS_NEW_MOVE",
            "registry exhaustion witness is validated; no registered move applies beyond it under the frozen move registry",
        )
    if history_has_move(state, "validate_no_legacy_ambiguous_fields.v0"):
        return (
            "STOP_NEEDS_NEW_MOVE",
