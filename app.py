...
TYPED_READY_CHECKPOINT_CODE = "TYPED_STATE_READY"
TYPED_READY_TERMINAL_HALT_CODE = "STOP_NEEDS_NEW_MOVE"
...
def validate_regime(regime: dict[str, Any]) -> list[str]:
    ...
    required_halts = {
        ...
        "REGIME_MISMATCH",
        "NO_APPLICABLE_MOVE",
        TYPED_READY_TERMINAL_HALT_CODE,
        "STEP_LIMIT_EXCEEDED",
    }
    ...
    if isinstance(halt_codes, list) and TYPED_READY_CHECKPOINT_CODE in halt_codes:
...
def has_checkpoint(state: dict[str, Any], checkpoint_code: str) -> bool:
    history = state.get("history") if isinstance(state.get("history"), list) else []
    for row in history:
        if not isinstance(row, dict):
            continue
        if row.get("outcome") == "checkpoint" and row.get("checkpoint_code") == checkpoint_code:
            return True
    return False
...
def move_checkpoint_typed_state_applies(state: dict[str, Any], regime: dict[str, Any]) -> tuple[bool, str]:
    ...
    if has_checkpoint(state, TYPED_READY_CHECKPOINT_CODE):
        return False, "typed-ready checkpoint already recorded"
...
def build_typed_record(state: dict[str, Any], regime: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_next_moves": ["checkpoint_typed_state.v0"],
        "stop_conditions": [
            "INVALID_STATE",
            "REGIME_MISMATCH",
            "NO_APPLICABLE_MOVE",
...
def apply_checkpoint_typed_state(
    ...
    new_state = append_history(
        state,
        {
            ...
            "outcome": "checkpoint",
            "checkpoint_code": TYPED_READY_CHECKPOINT_CODE,
            "halt_code": None,
            "reason": reason,
        },
    )
    new_state["status"] = "typed_ready"
    return new_state, None
...
def verdict_for_halt(halt_code: str) -> str:
    # No halt code is treated as proof success. The receipt is a typed witness
    # that the runner stopped at an explicit boundary.
    return "RUN_WITNESS_HALTED"


def checkpoints_from_trace(trace: list[dict[str, Any]]) -> list[str]:
    checkpoints: list[str] = []
    for row in trace:
        if not isinstance(row, dict):
            continue
        code = row.get("checkpoint_code")
        if isinstance(code, str) and code and code not in checkpoints:
            checkpoints.append(code)
    return checkpoints


def no_applicable_move_halt(state: dict[str, Any]) -> tuple[str, str]:
    if isinstance(state.get("typed_record"), dict) and has_checkpoint(state, TYPED_READY_CHECKPOINT_CODE):
        return (
            TYPED_READY_TERMINAL_HALT_CODE,
            "typed-ready checkpoint has fired; no registered move applies after checkpoint",
        )
...
def run_machine(
    ...
    for step in range(MAX_STEPS):
        ...
        if selected is None:
            halt_code, halt_reason = no_applicable_move_halt(current)
            trace.append(
                {
                    "step": step,
        ...
        history = after.get("history") if isinstance(after.get("history"), list) else []
        if history and isinstance(history[-1], dict):
            maybe_checkpoint = history[-1].get("checkpoint_code")
            if isinstance(maybe_checkpoint, str) and maybe_checkpoint:
                checkpoint_code = maybe_checkpoint
        ...
        if checkpoint_code is not None:
            trace_row["checkpoint_code"] = checkpoint_code
...
def projection_ascii_graph(receipt: dict[str, Any]) -> str:
    ...
    for row in trace:
        ...
        checkpoint_code = row.get("checkpoint_code")
        if halt_code:
            after_label = f"{after_sig} [{halt_code}]"
        elif checkpoint_code:
            after_label = f"{after_sig} [checkpoint:{checkpoint_code}]"
        else:
            after_label = after_sig
...
def projection_pattern_hints(
    ...
    for row in trace:
        ...
        if after is not None:
            after_sig = str(after)
            if not state_path or state_path[-1] != after_sig:
    ...
    return {
        ...
        "typed_ready_checkpoint": "yes"
        if TYPED_READY_CHECKPOINT_CODE in checkpoints_from_trace(trace)
        else "no",
    }
